...
This worksheet is built for one kind of proof: choose the cached name pointer printed at `__d_add`, then trace it forward to `do_filp_open return` and later `d_lookup` hits, and trace it backward to `__d_alloc return` and the copy source. Every step is either “same address”, “NULL”, or a subtraction that equals a path prefix length. The blanks are arranged around `__d_alloc` and `__d_add` because those are the only steps where the name address can change. Fill the numbers first, then draw the tree from the middle out; that is the only order that prevents hand‑waving.
...
sudo ./<program>
sleep 2
sudo dmesg | rg -n "t_e.txt|t_m.txt|l_m.txt|l_e.txt|a.txt|d_lookup entry|d_lookup return|__d_lookup_rcu|__d_alloc|__d_add|do_filp_open|d_delete|__dentry_kill"
```
...
- do_filp_open return pointer: may be absent if open failed
...
└─ do_filp_open entry 0x________
   └─ d_lookup entry (hash=____, len=5, name=a.txt)
      ...
      │           └─ do_filp_open return 0x________ | a.txt (if printed)
...
└─ d_lookup return NULL
   └─ __d_alloc return 0x________
      └─ __d_add entry 0x________ | t_e.txt
         └─ do_filp_open return 0x________ | t_e.txt
0x________ ≠ 0x________  OR  0x________ = 0x________
```
...
- compare pointers: post-eviction vs pre-eviction
