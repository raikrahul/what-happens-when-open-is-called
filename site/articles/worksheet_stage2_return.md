...
sudo dmesg -C
sudo rmmod trace_do_filp_open
sudo insmod ../../drivers/trace_do_filp_open/trace_do_filp_open.ko target_comm=te_miss
sudo ./te_miss
sleep 2
sudo dmesg | rg -n "t_e.txt|d_lookup entry|d_lookup return|__d_alloc|__d_add|do_filp_open"
```

TEST 2: MATRIX (split programs)

FILE A (t_e.txt, first open after drop_caches):
Why: clean miss path with a /tmp prefix and offset +5.
Run: ./te_miss
- do_filp_open entry pointer = 0xffff8b1491175020 | /tmp/t_e.txt
- d_lookup entry hash 3583106372 len 7 name t_e.txt
- d_lookup return: NULL
- __d_alloc entry pointer = 0xffff8b1491175025
- __d_alloc return pointer = 0xffff8b148a129038
- __d_add entry pointer = 0xffff8b148a129038 | t_e.txt
- do_filp_open return pointer = 0xffff8b148a129038 | t_e.txt
- offset check: (0xffff8b1491175020 + 5) == 0xffff8b1491175025

FILE B (t_m.txt, missing):
Why: miss+insert for a missing /tmp name.
Run: ./tm_miss
- do_filp_open entry pointer = 0xffff8b1489ae4020 | /tmp/t_m.txt
- d_lookup entry hash 502501587 len 7 name t_m.txt
- d_lookup return: NULL
- __d_alloc return pointer = 0xffff8b14d0f037b8
- __d_add entry pointer = 0xffff8b14d0f037b8 | t_m.txt

FILE C (l_m.txt, missing):
Why: miss+insert with no prefix (alloc entry equals string pointer).
Run: ./lm_miss
- do_filp_open entry pointer = 0xffff8b148eeb3020 | l_m.txt
- d_lookup entry hash 2257632620 len 7 name l_m.txt
- d_lookup return: NULL
- __d_alloc return pointer = 0xffff8b148042adb8
- __d_add entry pointer = 0xffff8b148042adb8 | l_m.txt

FILE D (a.txt, loopback ext2):
Why: same flow on ext2 with short name and /mnt/loopfs/ offset +12.
Run: ./a_miss
- do_filp_open entry pointer = 0xffff8b148256e020 | /mnt/loopfs/a.txt
- d_lookup entry hash 1021948357 len 5 name a.txt
- d_lookup return: NULL
- __d_alloc entry pointer = 0xffff8b148256e02c
- __d_alloc return pointer = 0xffff8b148d7ad4b8
- __d_add entry pointer = 0xffff8b148d7ad4b8 | a.txt
- do_filp_open return pointer = 0xffff8b148d7ad4b8 | a.txt
- offset check: (0xffff8b148256e020 + 12) == 0xffff8b148256e02c

CACHE HIT (before deletion):
Why: show pointer reuse on hit.
Run: ./hits
- d_lookup return pointer = 0xffff8b148d7a7938 | l_e.txt
- d_lookup return pointer = 0xffff8b148d7a71b8 | t_e.txt

DELETION (unlink):
Why: show d_delete removal for the same pointers.
Run: ./delete
- d_delete entry = 0xffff8b148d7a7938 | l_e.txt
- d_delete entry = 0xffff8b148d7a71b8 | t_e.txt

EVICTION (drop_caches):
Why: show __dentry_kill for cached names.
Run: ./evict
- do_filp_open entry/return for /proc/sys/vm/drop_caches
- __dentry_kill entry = 0xffff8b148d7a7938 | l_e.txt
- __dentry_kill entry = 0xffff8b148d7a71b8 | t_e.txt

REBUILD (after eviction, t_e.txt):
Why: show new or reused pointer after eviction.
Run: ./rebuild
- d_lookup return: NULL
- __d_alloc return pointer = 0xffff8b1484f8cb78
- __d_add entry pointer = 0xffff8b1484f8cb78 | t_e.txt
- do_filp_open return pointer = 0xffff8b1484f8cb78 | t_e.txt
- inequality check: 0xffff8b1484f8cb78 != 0xffff8b148d7a71b8
...
- do_filp_open return pointer = 0xffff8b14805a3c38 | l_e.txt
- inequality check: 0xffff8b14805a3c38 != 0xffff8b148d7a7938

Why these commands exist (do not skip):
- `sudo dmesg -C` clears the kernel ring buffer so the next lines are only from this run.
- `sudo insmod ... target_comm=te_miss` restricts probe output to the active program name.
- `sudo ./<program>` is required when the program writes to `/proc/sys/vm/drop_caches`.
- `sleep 2` allows delayed probe lines (lookups, eviction) to finish.
