...
Probe map (side-by-side)

| Probe | Observed field | Purpose |
| --- | --- | --- |
| do_filp_open entry | input filename pointer + string | capture kernel-side name at entry |
| __d_alloc entry | copy source pointer | capture name copy source |
| __d_alloc return | copy destination pointer | capture name copy destination |
| do_filp_open return | returned file name pointer | compare return pointer to copies and hits |
| d_lookup entry | hash, length, name | record lookup key |
| d_lookup return | dentry name pointer or NULL | detect hit vs miss |
| __d_lookup entry | hash, length, name | internal lookup path key |
| __d_lookup_rcu entry | hash, length, name | RCU lookup path key |
| full_name_hash return | hash, length, salt, name | record hash computation (not observed in this run) |
| __d_add entry | dentry name pointer | record cache insert |
| d_delete entry | dentry name pointer | record unlink removal |
| __dentry_kill entry | dentry name pointer | record eviction via drop_caches |
