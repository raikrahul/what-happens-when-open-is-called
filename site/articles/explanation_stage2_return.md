...
Kernel evidence:
do_filp_open entry pointer = 0xffff8bd543616020 | /tmp/t_e.txt
d_lookup entry: hash 1830572521 length 7 name t_e.txt
d_lookup return: NULL
__d_alloc entry pointer = 0xffff8bd543616025
__d_alloc return pointer = 0xffff8bd5451dd878
__d_add entry: 0xffff8bd5451dd878 | t_e.txt
do_filp_open return pointer = 0xffff8bd5451dd878 | t_e.txt

Pointer meanings (t_e.txt):
- do_filp_open entry pointer 0xffff8bd543616020 points to the kernel-side string "/tmp/t_e.txt" used for this open call.
...
- __d_alloc entry pointer 0xffff8bd543616025 points to the basename start inside the same string (after "/tmp/").
- __d_alloc return pointer 0xffff8bd5451dd878 points to newly allocated dentry name storage.
- __d_add entry pointer 0xffff8bd5451dd878 shows that same storage inserted into the dcache hash.
- do_filp_open return pointer 0xffff8bd5451dd878 shows the returned file points at that dentry name storage.
...
"/tmp/" length = 5
0xffff8bd5451dd878 = __d_alloc return pointer
0xffff8bd5451dd878 = __d_add entry pointer
0xffff8bd5451dd878 = do_filp_open return pointer
...
Kernel evidence:
do_filp_open entry pointer = 0xffff8bd543616020 | /tmp/t_m.txt
d_lookup entry: hash 2543581516 length 7 name t_m.txt
d_lookup return: NULL
__d_alloc entry pointer = 0xffff8bd543616025
__d_alloc return pointer = 0xffff8bd5451dd7b8
__d_add entry: 0xffff8bd5451dd7b8 | t_m.txt

Pointer meanings (t_m.txt):
- do_filp_open entry pointer 0xffff8bd543616020 points to "/tmp/t_m.txt" in kernel memory for this open.
...
- __d_alloc entry pointer 0xffff8bd543616025 points to basename start after "/tmp/".
- __d_alloc return pointer 0xffff8bd5451dd7b8 is the newly allocated dentry name storage.
- __d_add entry pointer 0xffff8bd5451dd7b8 inserts that name storage into the dcache as a negative entry.
...
Kernel evidence:
do_filp_open entry pointer = 0xffff8bd543616020 | l_m.txt
d_lookup entry: hash 1675155717 length 7 name l_m.txt
d_lookup return: NULL
__d_alloc entry pointer = 0xffff8bd543616020
__d_alloc return pointer = 0xffff8bd5451dd6f8
__d_add entry: 0xffff8bd5451dd6f8 | l_m.txt

Pointer meanings (l_m.txt):
- do_filp_open entry pointer 0xffff8bd543616020 points to "l_m.txt" for this open.
- d_lookup entry hash 1675155717 and length 7 use basename "l_m.txt" as the key.
- d_lookup return: NULL shows no cached dentry matched.
- __d_alloc entry pointer 0xffff8bd543616020 equals the string start (no prefix).
- __d_alloc return pointer 0xffff8bd5451dd6f8 is the newly allocated dentry name storage.
- __d_add entry pointer 0xffff8bd5451dd6f8 inserts that name storage into the dcache as a negative entry.
...
Kernel evidence:
do_filp_open entry pointer = 0xffff8bd543616020 | /mnt/loopfs/a.txt
d_lookup entry: hash 3711754354 length 5 name a.txt
d_lookup return: NULL
__d_alloc entry pointer = 0xffff8bd54361602c
__d_alloc return pointer = 0xffff8bd5451ddcf8
__d_add entry: 0xffff8bd5451ddcf8 | a.txt
do_filp_open return pointer = 0xffff8bd5451ddcf8 | a.txt

Pointer meanings (a.txt):
- do_filp_open entry pointer 0xffff8bd543616020 points to "/mnt/loopfs/a.txt" in kernel memory for this open.
- d_lookup entry hash 3711754354 and length 5 use basename "a.txt" as the key.
- d_lookup return: NULL shows no cached dentry matched.
- __d_alloc entry pointer 0xffff8bd54361602c points to basename start after "/mnt/loopfs/".
- __d_alloc return pointer 0xffff8bd5451ddcf8 is the newly allocated dentry name storage.
- __d_add entry pointer 0xffff8bd5451ddcf8 inserts that name storage into the dcache.
- do_filp_open return pointer 0xffff8bd5451ddcf8 shows the returned file points at that dentry name storage.

Derivation:
0xffff8bd54361602c - 0xffff8bd543616020 = 0xC = 12
"/mnt/loopfs/" length = 12
0xffff8bd5451ddcf8 = __d_alloc return pointer
0xffff8bd5451ddcf8 = __d_add entry pointer
0xffff8bd5451ddcf8 = do_filp_open return pointer
...
d_lookup return: 0xffff8bd54422d0f8 | l_e.txt

Pointer meanings (cache hits):
- d_lookup return pointer 0xffff8bd54422d0f8 for l_e.txt matches the earlier return pointer for l_e.txt, so the cached dentry is reused.
- d_lookup return pointer 0xffff8bd5451dd878 for t_e.txt matches the earlier return pointer for t_e.txt, so the cached dentry is reused.

d_lookup entry: hash 1830572521 length 7 name t_e.txt
d_lookup return: 0xffff8bd5451dd878 | t_e.txt
...
Kernel evidence:
d_delete entry: 0xffff8bd54422d0f8 | l_e.txt
d_delete entry: 0xffff8bd5451dd878 | t_e.txt
no d_drop entry lines observed in this run

Pointer meanings (deletion):
- d_delete entry pointer 0xffff8bd54422d0f8 names the cached l_e.txt dentry that was removed by unlink.
- d_delete entry pointer 0xffff8bd5451dd878 names the cached t_e.txt dentry that was removed by unlink.
...
Kernel evidence:
do_filp_open entry pointer = 0xffff8bd543616020 | /proc/sys/vm/drop_caches
do_filp_open return pointer = 0xffff8bd6fce05db8 | drop_caches
__dentry_kill entry: 0xffff8bd54422d0f8 | l_e.txt
__dentry_kill entry: 0xffff8bd5451dd878 | t_e.txt
...
- __dentry_kill entry pointers 0xffff8bd54422d0f8 and 0xffff8bd5451dd878 identify the exact l_e.txt and t_e.txt dentries reclaimed by eviction.
...
d_lookup entry: hash 1830572521 length 7 name t_e.txt
__d_alloc entry pointer = 0xffff8bd543616025
__d_alloc return pointer = 0xffff8bd5451ddb78
__d_add entry: 0xffff8bd5451ddb78 | t_e.txt
do_filp_open return pointer = 0xffff8bd5451ddb78 | t_e.txt

Restated pre-eviction t_e.txt pointer used for inequality:
do_filp_open return pointer = 0xffff8bd5451dd878 | t_e.txt

Pointer meanings (rebuild t_e.txt):
- __d_alloc entry pointer 0xffff8bd543616025 points to basename "t_e.txt" in the reopened path.
- __d_alloc return pointer 0xffff8bd5451ddb78 is the new dentry name storage after eviction.
- __d_add entry pointer 0xffff8bd5451ddb78 inserts the rebuilt name into the cache.
- do_filp_open return pointer 0xffff8bd5451ddb78 confirms the returned file points at the rebuilt name storage.

Derivation:
0xffff8bd5451ddb78 != 0xffff8bd5451dd878
...
Kernel evidence:
do_filp_open entry pointer = 0xffff8bd543616020 | l_e.txt
__d_lookup_rcu entry: hash 399720033 length 7 name l_e.txt
do_filp_open return pointer = 0xffff8bd562b4f878 | l_e.txt

Restated pre-eviction l_e.txt pointer used for inequality:
do_filp_open return pointer = 0xffff8bd54422d0f8 | l_e.txt

Pointer meanings (post-eviction l_e.txt):
- __d_lookup_rcu entry shows the lookup key for l_e.txt after eviction.
- do_filp_open return pointer 0xffff8bd562b4f878 is a new name pointer (not equal to pre-eviction pointer 0xffff8bd54422d0f8).

Derivation:
0xffff8bd562b4f878 != 0xffff8bd54422d0f8
...
d_lookup entry: hash 729609125 length 64 name test_file_very_long_name_to_force_external_allocation_1770406343
d_lookup return: NULL
__d_alloc entry pointer = 0xffff8bd543616020
__d_alloc return pointer = 0xffff8bd7a3191618
__d_add entry pointer = 0xffff8bd7a3191618 | test_file_very_long_name_to_force_external_allocation_1770406343
do_filp_open return pointer = 0xffff8bd7a3191618 | test_file_very_long_name_to_force_external_allocation_1770406343

Pointer meanings (long filename):
- do_filp_open entry pointer 0xffff8bd543616020 points to the long filename string in kernel memory.
...
- __d_alloc entry pointer 0xffff8bd543616020 equals the full string start (no prefix to skip).
- __d_alloc return pointer 0xffff8bd7a3191618 is the new dentry name storage for the long string.
- __d_add entry pointer 0xffff8bd7a3191618 inserts that name into the cache.
- do_filp_open return pointer 0xffff8bd7a3191618 confirms the returned file points to that name storage.
...
d_lookup entry: hash 729609125 length 64 name test_file_very_long_name_to_force_external_allocation_1770406343
d_lookup return pointer = 0xffff8bd7a3191618
...
__d_alloc entry pointer = 0xffff8bd543616020
__d_alloc return pointer = 0xffff8bd7a3191618
__d_add entry: 0xffff8bd7a3191618 | test_file_very_long_name_to_force_external_allocation_1770406343
do_filp_open return pointer = 0xffff8bd7a3191618 | test_file_very_long_name_to_force_external_allocation_1770406343

Derivation:
0xffff8bd7a3191618 = __d_alloc return pointer
0xffff8bd7a3191618 = __d_add entry pointer
0xffff8bd7a3191618 = do_filp_open return pointer
...
#1. Call. do_filp_open entry. Values: name pointer 0xffff8bd543616020, name string /tmp/t_e.txt. Data: user-space open("/tmp/t_e.txt") reached kernel. Work: begin lookup for basename t_e.txt. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: will return to caller with file pointer.
...
#6. Call. __d_alloc entry. Values: name pointer 0xffff8bd543616025. Data: points to basename start in /tmp/t_e.txt. Work: allocate dentry name storage and copy. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns new dentry pointer.
#7. Return. __d_alloc return. Values: name pointer 0xffff8bd5451dd878. Data: newly allocated dentry name storage for t_e.txt. Work: allocate completed. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: control continues to insert into dcache.
#8. Call. __d_add entry. Values: name pointer 0xffff8bd5451dd878, name t_e.txt. Data: inserting new dentry. Work: insert into dcache hash. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to open path.
#9. Return. do_filp_open. Values: name pointer 0xffff8bd5451dd878, name t_e.txt. Data: returned file points to newly allocated name storage. Work: open completes. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to user space.
#10. Call. do_filp_open entry. Values: name pointer 0xffff8bd543616020, name /tmp/t_m.txt. Data: user-space open of missing file. Work: begin lookup for basename t_m.txt. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: will return with error or file.
...
#15. Call. __d_alloc entry. Values: name pointer 0xffff8bd543616025. Data: points to basename start in /tmp/t_m.txt. Work: allocate dentry name storage and copy. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns new dentry pointer.
#16. Return. __d_alloc return. Values: name pointer 0xffff8bd5451dd7b8. Data: newly allocated dentry name storage for t_m.txt. Work: allocation completed. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: control continues to insert.
#17. Call. __d_add entry. Values: name pointer 0xffff8bd5451dd7b8, name t_m.txt. Data: inserting negative dentry. Work: insert into dcache hash. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to open path.
#18. Call. do_filp_open entry. Values: name pointer 0xffff8bd543616020, name l_m.txt. Data: user-space open of missing file. Work: begin lookup for basename l_m.txt. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: will return with error or file.
...
#22. Call. __d_alloc entry. Values: name pointer 0xffff8bd543616020. Data: points to basename start in l_m.txt. Work: allocate dentry name storage and copy. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns new dentry pointer.
#23. Return. __d_alloc return. Values: name pointer 0xffff8bd5451dd6f8. Data: newly allocated dentry name storage for l_m.txt. Work: allocation completed. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: control continues to insert.
#24. Call. __d_add entry. Values: name pointer 0xffff8bd5451dd6f8, name l_m.txt. Data: inserting negative dentry. Work: insert into dcache hash. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to open path.
#25. Call. do_filp_open entry. Values: name pointer 0xffff8bd543616020, name /mnt/loopfs/a.txt. Data: user-space open on loopback ext2. Work: begin lookup for basename a.txt. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: will return with file pointer.
...
#31. Call. __d_alloc entry. Values: name pointer 0xffff8bd54361602c. Data: points to basename start in /mnt/loopfs/a.txt. Work: allocate dentry name storage and copy. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns new dentry pointer.
#32. Return. __d_alloc return. Values: name pointer 0xffff8bd5451ddcf8. Data: newly allocated dentry name storage for a.txt. Work: allocation completed. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: control continues to insert.
#33. Call. __d_add entry. Values: name pointer 0xffff8bd5451ddcf8, name a.txt. Data: inserting new dentry. Work: insert into dcache hash. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to open path.
#34. Return. do_filp_open. Values: name pointer 0xffff8bd5451ddcf8, name a.txt. Data: returned file points to newly allocated name storage. Work: open completes. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to user space.
#35. Call. do_filp_open entry. Values: name pointer 0xffff8bd543616020, name l_e.txt. Data: user-space open of existing file. Work: lookup for l_e.txt. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: will return with file pointer.
...
#37. Return. d_lookup return. Values: pointer 0xffff8bd54422d0f8, name l_e.txt. Data: cache hit. Work: reuse cached dentry. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: control returns to open path.
#38. Return. do_filp_open. Values: name pointer 0xffff8bd54422d0f8, name l_e.txt. Data: open returns cached dentry name storage. Work: open completes. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to user space.
#39. Call. d_delete entry. Values: pointer 0xffff8bd54422d0f8, name l_e.txt. Data: unlink removal. Work: delete dentry from hash. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to unlink path.
#40. Call. d_delete entry. Values: pointer 0xffff8bd5451dd878, name t_e.txt. Data: unlink removal. Work: delete dentry from hash. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to unlink path.
#41. Call. do_filp_open entry. Values: name pointer 0xffff8bd543616020, name /proc/sys/vm/drop_caches. Data: control write to drop caches. Work: open drop_caches for eviction. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to user space.
...
#43. Call. __dentry_kill entry. Values: pointer 0xffff8bd54422d0f8, name l_e.txt. Data: eviction of cached dentry. Work: reclaim dentry. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to shrinker.
#44. Call. __dentry_kill entry. Values: pointer 0xffff8bd5451dd878, name t_e.txt. Data: eviction of cached dentry. Work: reclaim dentry. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to shrinker.
#45. Call. do_filp_open entry. Values: name pointer 0xffff8bd543616020, name l_e.txt. Data: post-eviction open. Work: lookup after eviction. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns with file pointer.
...
#47. Return. do_filp_open. Values: name pointer 0xffff8bd562b4f878, name l_e.txt. Data: post-eviction return pointer differs from pre-eviction pointer 0xffff8bd54422d0f8. Work: open completes. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to user space.
#48. Call. do_filp_open entry. Values: name pointer 0xffff8bd543616020, name /tmp/t_e.txt. Data: post-eviction open. Work: lookup after eviction. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns with file pointer.
...
#51. Call. __d_alloc entry. Values: name pointer 0xffff8bd543616025. Data: basename pointer. Work: allocate new name storage. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns new dentry pointer.
#52. Return. __d_alloc return. Values: name pointer 0xffff8bd5451ddb78. Data: new name storage for t_e.txt. Work: allocation completed. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: control continues to insert.
#53. Call. __d_add entry. Values: name pointer 0xffff8bd5451ddb78, name t_e.txt. Data: insert new dentry. Work: insert into dcache. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to open path.
#54. Return. do_filp_open. Values: name pointer 0xffff8bd5451ddb78, name t_e.txt. Data: rebuilt pointer differs from pre-eviction pointer 0xffff8bd5451dd878. Work: open completes. Errors: none. Caller line: not recorded. Current line: not recorded. Resume: returns to user space.
...
```text
open("/tmp/t_e.txt")
  -> do_filp_open entry 0xffff8bd543616020
  -> d_lookup hash 1830572521 len 7 "t_e.txt" -> NULL
  -> __d_alloc entry 0xffff8bd543616025
  -> __d_alloc return 0xffff8bd5451dd878
  -> __d_add entry 0xffff8bd5451dd878
  -> do_filp_open return 0xffff8bd5451dd878
```
...
open("/mnt/loopfs/a.txt")
  ...
  -> __d_alloc entry 0xffff8bd54361602c
  -> __d_alloc return 0xffff8bd5451ddcf8
  -> __d_add entry 0xffff8bd5451ddcf8
  -> do_filp_open return 0xffff8bd5451ddcf8
```

Diagram 3 — cache hit for l_e.txt before deletion
```text
open("l_e.txt")
  -> d_lookup hash 399720033 len 7 "l_e.txt" -> 0xffff8bd54422d0f8
  -> do_filp_open return 0xffff8bd54422d0f8
```

Diagram 4 — unlink deletion + eviction
```text
unlink("l_e.txt") -> d_delete 0xffff8bd54422d0f8
unlink("/tmp/t_e.txt") -> d_delete 0xffff8bd5451dd878
drop_caches -> __dentry_kill 0xffff8bd54422d0f8 (l_e.txt)
drop_caches -> __dentry_kill 0xffff8bd5451dd878 (t_e.txt)
```
...
open("/tmp/t_e.txt") after drop_caches
  -> d_lookup hash 1830572521 len 7 "t_e.txt" -> NULL
  -> __d_alloc return 0xffff8bd5451ddb78
  -> __d_add entry 0xffff8bd5451ddb78
  -> do_filp_open return 0xffff8bd5451ddb78
```
...
open("l_e.txt") after drop_caches
  ...
  -> do_filp_open return 0xffff8bd562b4f878
```
