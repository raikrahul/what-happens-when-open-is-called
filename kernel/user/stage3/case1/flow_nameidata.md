...
## Visual Flow of Initialization

```mermaid
graph TD
    A[do_filp_open] -->|Allocates stack var| B[struct nameidata nd]
    B -->|Contains| C[GARBAGE Memory]
    
    A -->|Calls| D[set_nameidata &nd, dfd, name, NULL]
    
    D -->|Calls| E[__set_nameidata]
    
    E -->|Sets| F[nd.dfd = -100]
    E -->|Sets| G[nd.filename = name]
    E -->|Sets| H[nd.stack = nd.internal]
    E -->|Sets| I[current->nameidata = &nd]
    E -->|Reads old| J[old = current->nameidata]
    J -->|Old was NULL| K[nd.saved = NULL]

    D -->|Checks root arg| L{root == NULL?}
    L -- Yes --> M[Skip root setup]
    M -->|Result| N[nd.root remains GARBAGE]
    
    A -->|Calls| O[path_openat &nd]
    O -->|Calls| P[path_init nd]
    P -->|Sets| Q[nd.root = current->fs->root]
    P -->|Sets| R[nd.path = current->fs->pwd]
