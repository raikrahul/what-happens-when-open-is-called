...
static char target_comm[TASK_COMM_LEN] = "demo_o_path";
...
static int probe_entry_do_filp_open(struct kprobe *p, struct pt_regs *regs)
{
    ...
    pr_info("[trace_demo_o_path] do_filp_open.entry pid=%d comm=%s dfd=%lld pathname=%px op=%px\n",
...
static int probe_ret_do_filp_open(struct kretprobe_instance *ri, struct pt_regs *regs)
{
    ...
    pr_info("[trace_demo_o_path] do_filp_open.ret pid=%d comm=%s ret=%px\n",
...
static int probe_entry___d_alloc(struct kprobe *p, struct pt_regs *regs)
{
    ...
    pr_info("[trace_demo_o_path] __d_alloc.entry pid=%d comm=%s sb=%px name_qstr=%px\n",
...
static int probe_ret___d_alloc(struct kretprobe_instance *ri, struct pt_regs *regs)
{
    ...
    pr_info("[trace_demo_o_path] __d_alloc.ret pid=%d comm=%s dentry=%px\n",
...
static int __init trace_demo_o_path_init(void)
{
    int rc;
    ...
    pr_info("[trace_demo_o_path] loaded target_comm=%s user=kernel/user/stage3/case1/demo_o_path/demo_o_path.c\n",
...
static void __exit trace_demo_o_path_exit(void)
{
    unregister_kretprobe(&krp___d_alloc);
    unregister_kprobe(&kp___d_alloc);
    unregister_kretprobe(&krp_do_filp_open);
    unregister_kprobe(&kp_do_filp_open);
    pr_info("[trace_demo_o_path] unloaded\n");
}

module_init(trace_demo_o_path_init);
module_exit(trace_demo_o_path_exit);
MODULE_LICENSE("GPL");
MODULE_AUTHOR("stage3-automation");
MODULE_DESCRIPTION("Dedicated Stage3 trace driver for kernel/user/stage3/case1/demo_o_path/demo_o_path.c");
